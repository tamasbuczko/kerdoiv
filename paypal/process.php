...
if($_POST) //Post Data received from product list page.
{
	...
	#$ItemPrice 		= $_POST["itemprice"]; //Item Price (ink�bb m�r adatb�zisb�l)
	...
	$padata = 	'&METHOD=SetExpressCheckout'.
				...
				*/
				...
				'&LOGOIMG=http://www.citypixel.hu/graphics/logo_paypal.png'. //site logo
...
if(isset($_GET["token"]) && isset($_GET["PayerID"]))
{
	...
	if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
	{
			...
			$cikk_php_html .= 'Tranzakciós ID: '.urldecode($httpParsedResponseAr["PAYMENTINFO_0_TRANSACTIONID"]);
				...
				*/
				...
				{
					$cikk_php_html .= '<div style="color:green">A fizetés sikeresen megtörtént!</div>';
				}
				elseif('Pending' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"])
				{
					$cikk_php_html .= '<div style="color:red">Transaction Complete, but payment is still pending! '.
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
				{
					...
					*/
					...
					$query = "INSERT ".$_SESSION[adatbazis_etag]."_vasarlasok
					('vasarlo_id', 'csomag_id', 'idopont', 'osszeg', 'status')
					VALUES
					('12', '$ItemDesc', '$idopont', '$GrandTotal', '1')";
					mysql_query($query);
					
					$cikk_php_html .= '<pre>';
					###$cikk_php_html .= print_r($httpParsedResponseAr);
					$cikk_php_html .= '</pre>';
				} else  {
					$cikk_php_html .= '<div style="color:red"><b>GetTransactionDetails failed:</b>'.urldecode($httpParsedResponseAr["L_LONGMESSAGE0"]).'</div>';
					$cikk_php_html .= '<pre>';
					$cikk_php_html .= print_r($httpParsedResponseAr);
					$cikk_php_html .= '</pre>';
			...
			$cikk_php_html .= '<div style="color:red"><b>Error : </b>'.urldecode($httpParsedResponseAr["L_LONGMESSAGE0"]).'</div>';
			$cikk_php_html .= '<pre>';
			$cikk_php_html .= print_r($httpParsedResponseAr);
			$cikk_php_html .= '</pre>';
	}
